DROP FUNCTION IF EXISTS set_asn_moves_as_cancel_training(varchar, varchar);
CREATE OR REPLACE FUNCTION set_asn_moves_as_cancel_training(_store_code character varying, date_now character varying)
RETURNS void
LANGUAGE plpgsql
AS $$
    DECLARE
    ...
    BEGIN
        ...
        RAISE notice 'TRIGER STATUS: Running cancel Stock Move';
        ...
        RAISE notice 'TRIGER STATUS: Running cancel Stock Move Line';
        ...
        RAISE notice 'TRIGER STATUS: Running cancel Stock Quant';
        ...
        FROM (SELECT product_id, location_dest_id, SUM(product_uom_qty) as product_uom_qty
            ...
            WHERE origin='ASN_DATA' AND location_dest_id=locations.location_dest_id
                AND date::DATE<date date_now AND state='cancel'
        ...
        WHERE origin='ASN_DATA' AND sm.location_dest_id=locations.location_dest_id
            AND date::DATE<date_now AND state='cancel' AND q.id IS NULL
        ...
        WHERE origin='ASN_DATA' AND sm.location_id=locations.location_id
            AND date::DATE<date date_now AND state='cancel' AND q.id IS NULL
        GROUP BY sm.product_id, sm.location_id;

        RAISE notice 'TRIGER STATUS: Running cancel master';
