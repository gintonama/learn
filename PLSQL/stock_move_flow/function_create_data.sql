...
AS $$
    BEGIN
		...
		INSERT INTO deliver_transactions (
            accounting_date, quantity, unit_price, partner_id, warehouse_id, rule_id,
...
AS $$
    DECLARE
        ...
        dev_tran record;
    begin
        FOR dev_tran in select product_code, picking_id, warehouse_id from deliver_transactions
        LOOP
            ...
            SELECT sum(quantity) FROM stock_quant
            WHERE product_id in (SELECT id FROM product_product WHERE default_code = dev_tran.product_code)  
                AND location_id in (SELECT lot_stock_id FROM stock_warehouse WHERE id = dev_tran.warehouse_id)
            INTO qty_on_hand;
...
AS $$
    DECLARE
        ...
        moves_dest_id int;
    BEGIN
        FOR temp_data IN SELECT array_agg(id) as ids, count(*) as ln FROM deliver_transactions GROUP BY product_code
		LOOP
			raise notice 'check len data %', temp_data;
			counter := 1;
			WHILE counter < temp_data.ln LOOP
				SELECT move_id FROM deliver_transactions dt WHERE id = temp_data.ids[counter] INTO moves_orig_id;
				SELECT move_id FROM deliver_transactions dt WHERE id = temp_data.ids[counter+1] INTO moves_dest_id;
...
AS $$
    BEGIN
        ...
        JOIN stock_picking sp ON sp.id = dt.picking_id
        JOIN product_product pp ON pp.default_code = dt.product_code
        WHERE sequence_code = 'IN' OR product_on_hand > 0
        ON CONFLICT(id)
        DO NOTHING;
