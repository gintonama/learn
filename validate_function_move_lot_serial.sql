...
AS $$
    DECLARE
        ...
        dev_tran record;
    begin
        FOR dev_tran in select product_code, picking_id, product_lot_code from deliver_transactions
        LOOP
            ...
            WHERE sml.lot_id = spl.id
                ...
                AND sml.lot_id in (SELECT spl.id FROM stock_production_lot spl, product_product pp WHERE spl.product_id = pp.id AND spl.name = dev_tran.product_lot_code AND pp.default_code = dev_tran.product_code)  
                AND sml.location_dest_id in (SELECT location_src_id FROM deliver_transactions WHERE picking_id = dev_tran.picking_id)
                AND sml.state = 'done' 
            INTO move_in;
            ...
            WHERE sml.lot_id = spl.id
                AND sml.product_id in (SELECT id FROM product_product WHERE default_code = dev_tran.product_code)  
                AND sml.lot_id in (SELECT spl.id FROM stock_production_lot spl, product_product pp WHERE spl.product_id = pp.id AND spl.name = dev_tran.product_lot_code AND pp.default_code = dev_tran.product_code)
                AND sml.location_dest_id not in (SELECT location_src_id FROM deliver_transactions WHERE picking_id = dev_tran.picking_id)
                AND state = 'done' 
            INTO move_out;
            ...
            WHERE pp.id = sq.product_id
                ...
                AND sq.lot_id in (SELECT spl.id FROM stock_production_lot spl, product_product pp WHERE spl.product_id = pp.id AND spl.name = dev_tran.product_lot_code AND pp.default_code = dev_tran.product_code)  
                AND sq.location_id in (SELECT location_src_id FROM deliver_transactions WHERE picking_id = dev_tran.picking_id)
            INTO quant_in;
